...
def _split_compound_clauses(text: str) -> list[str]:
    ...
    # Split on semicolons, comma-separated connectives, or coordinating conjunctions followed by a predicate clause
    clauses = re.split(
        r";|\s*(?:,\s*|\s+)(?:whereas|while|but also|where|wherein|proving that|which proves that|which proves|resulting in|which led to)\s+|\s*,\s*(?:and|but)\s+|\s+(?:and|but)\s+(?=[a-zA-Z0-9\s']+\b(?:is|was|are|were|has|have|had|will|can|could|did|does|reached|grew|dropped|increased|decreased)\b)",
        text,
        flags=re.IGNORECASE,
    )
