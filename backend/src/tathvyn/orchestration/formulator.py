...
class QueryFormulator:
    ...
    # Common stopwords to strip from initial queries
    _QUERY_STOPWORDS = frozenset({
        "the", "a", "an", "is", "was", "are", "were", "in", "on", "at",
        "of", "to", "for", "and", "or", "it", "its", "this", "that",
        "by", "with", "has", "have", "had", "be", "been", "being",
        "which", "who", "from", "as", "but", "not", "also",
    })
    ...
    def formulate_initial_queries(
        ...
        Produces natural, high-signal search queries without artificially restricting
        queries to single sites (which severely degrades retrieval recall).
        """
        queries: list[tuple[UUID, str]] = []
        # Collect top entities from all atomic claims for coreference resolution
        parent_entities: list[str] = []
        for ac in atomic_claims:
            for e in extract_named_entities(ac.text):
                if e["text"] not in parent_entities:
                    parent_entities.append(e["text"])

        for ac in atomic_claims[:max_queries]:
            raw = ac.text.strip().rstrip(".")
            clean_text = re.sub(r"\s+", " ", raw)

            # Coreference grounding: if sub-claim starts with pronoun (Its, Their, His, Her), prepend primary entity
            lower_words = clean_text.lower().split()
            if lower_words and lower_words[0] in ("its", "their", "his", "her") and parent_entities:
                clean_text = f"{parent_entities[0]} {clean_text}"
            ...
            if len(words) <= 12:
                ...
                filtered = [
                    w for w in words
                    if w.lower() not in self._QUERY_STOPWORDS or w.isupper()
                ]
                query_str = " ".join(filtered[:10]) if filtered else clean_text

            queries.append((ac.atomic_claim_id, query_str.strip()))
        return queries
