...
class AdaptiveResearchEngine:
    ...
    async def verify_stream(
        ...
        """Execute adaptive research verification with streaming stage events.

        Yields:
            dict: Event objects indicating pipeline progress stages and final completed verdict.
        """
        async for event in self.graph_runner.execute_research_stream(
            claim_text=claim_text,
            depth=depth,
            request_id=request_id,
        ):
