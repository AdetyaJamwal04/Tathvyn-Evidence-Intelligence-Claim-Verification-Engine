...
def _to_prob(val: float) -> float:
    """Safely bound cross-encoder logit output to [0.0, 1.0] using standard numerically stable sigmoid."""
    try:
        if val >= 0:
            z = math.exp(-val)
            prob = 1.0 / (1.0 + z)
        ...
        if model is not None:
            ...
            try:
                ...
                raw_scored = [
                    ...
                    for (pid, text), score in zip(passages, scores_list, strict=False)
                ]
