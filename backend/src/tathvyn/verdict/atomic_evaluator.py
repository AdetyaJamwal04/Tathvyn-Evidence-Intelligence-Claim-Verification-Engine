...
class AtomicClaimVerdictEvaluator:
    ...
    def evaluate_atomic_claim(
        ...
        # 1. Compute independent evidence weights
        w_sup = sum(
        ...
        # 2. Corroborated support: if support mass decisively outweighs contradiction
        ...
        # 3. Decisive refutation: if contradiction mass decisively outweighs support
        if w_con >= 0.40 and w_con >= 1.3 * max(w_sup, 0.01):
        ...
        # 4. Balanced opposing evidence (genuine empirical conflict)
        if evidence_state.unresolved_conflict or (w_sup >= 0.25 and w_con >= 0.25):
            return AtomicVerdictEvaluation(
                ...
                # Negative-evidence path: passages were found but none corroborate the claim.
        # For extraordinary factual claims (specific numbers, state-actor attribution, mass-casualty
        # events), the absence of confirmation from topic-relevant sources is itself refuting evidence.
        context_count = len(evidence_state.context_evidence)
        coverage = evidence_state.coverage_score

        if (context_count >= 2 or len(evidence_state.contradicting_evidence) > 0) and w_sup == 0.0:
            neg_conf = min(0.85, 0.72 + min(max(coverage, 0.1), 0.40) * 0.30)
            return AtomicVerdictEvaluation(
                verdict=AtomicClaimVerdict.REFUTED,
                support_weight=round(w_sup, 4),
                contradiction_weight=round(w_con, 4),
                confidence=round(neg_conf, 4),
                rationale=(
                    f"Authoritative multi-source search ({context_count} topic-relevant passages) found zero corroborating evidence. "
                    f"For extraordinary or specific empirical assertions, the complete absence of confirmation across "
                    f"credible public records refutes the claim (coverage={coverage:.2f})."
                ),
            )

        # Default fallback when evidence is genuinely absent
        return AtomicVerdictEvaluation(
